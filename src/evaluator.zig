...
const Value = union(ValueType) {
    ...
    function: struct { args: []const *const Node, env: *ValueMaps.Node },
...
pub const Evaluator = struct {
    ...
    pub fn evaluate(self: @This(), node: *const Node, env: *ValueMaps) EvaluationError!Value {
        ...
        return switch (node.token.tag) {
            ...
            .LEFT_PAREN, .LEFT_BRACE => try self.evaluate(node.args[0], env),
            ...
            .RIGHT_PAREN => blk: {
                ...
                switch (args[0]) {
                    ...
                    .function => |func| {
                        ...
                        if (self.evaluate(func.args[args.len], env)) |value| {
                            break :blk value;
            ...
            .FUN => .{ .nil = {
                const scope = env.first.?;
                try scope.data.put(
                    node.args[0].token.source(self.source),
                    .{ .function = .{ .args = node.args, .env = scope } },
                );
            } },
            .RETURN => blk: {
