...
pub const Evaluator = struct {
    ...
    pub fn evaluate(self: @This(), node: *const Node, env: *ValueMaps, expr: bool) !?Value {
        ...
        return switch (node.token.tag) {
            ...
            .LEFT_PAREN, .RETURN => if (node.args.len > 0) try self.evaluate(node.args[0], env, true) else .{ .nil = {} },
            .MINUS => if (node.args.len == 1) switch ((try self.evaluate(node.args[0], env, true)).?) {
                .number => |lhs| .{ .number = -lhs },
                else => error.OperandMustBeANumber,
            } else switch ((try self.evaluate(node.args[0], env, true)).?) {
                .number => |lhs| switch ((try self.evaluate(node.args[1], env, true)).?) {
                    .number => |rhs| .{ .number = lhs - rhs },
                    else => error.OperandsMustBeNumbers,
                },
                else => error.OperandsMustBeNumbers,
            },
            .PLUS => switch ((try self.evaluate(node.args[0], env, true)).?) {
                .number => |lhs| switch ((try self.evaluate(node.args[1], env, true)).?) {
                    .number => |rhs| .{ .number = lhs + rhs },
                    else => error.OperandsMustBeNumbers,
                },
                .string => |lhs| switch ((try self.evaluate(node.args[1], env, true)).?) {
                    .string => |rhs| blk: {
            ...
            .BANG => .{ .bool = !(try self.evaluate(node.args[0], env, true)).?.truthy() },
            .STAR => switch ((try self.evaluate(node.args[0], env, true)).?) {
                .number => |lhs| switch ((try self.evaluate(node.args[1], env, true)).?) {
                    .number => |rhs| .{ .number = lhs * rhs },
                    else => error.OperandsMustBeNumbers,
                },
            ...
            .SLASH => switch ((try self.evaluate(node.args[0], env, true)).?) {
                .number => |lhs| switch ((try self.evaluate(node.args[1], env, true)).?) {
                    .number => |rhs| .{ .number = lhs / rhs },
                    else => error.OperandsMustBeNumbers,
                },
            ...
            .LESS => switch ((try self.evaluate(node.args[0], env, true)).?) {
                .number => |lhs| switch ((try self.evaluate(node.args[1], env, true)).?) {
            ...
            .LESS_EQUAL => switch ((try self.evaluate(node.args[0], env, true)).?) {
                .number => |lhs| switch ((try self.evaluate(node.args[1], env, true)).?) {
                    .number => |rhs| .{ .bool = lhs <= rhs },
                    else => error.OperandsMustBeNumbers,
                },
            ...
            .GREATER => switch ((try self.evaluate(node.args[0], env, true)).?) {
                .number => |lhs| switch ((try self.evaluate(node.args[1], env, true)).?) {
                    .number => |rhs| .{ .bool = lhs > rhs },
                    else => error.OperandsMustBeNumbers,
                },
            ...
            .GREATER_EQUAL => switch ((try self.evaluate(node.args[0], env, true)).?) {
                .number => |lhs| switch ((try self.evaluate(node.args[1], env, true)).?) {
                    .number => |rhs| .{ .bool = lhs >= rhs },
                    else => error.OperandsMustBeNumbers,
                },
            ...
            .EQUAL_EQUAL => .{ .bool = switch ((try self.evaluate(node.args[0], env, true)).?) {
                .number => |lhs| switch ((try self.evaluate(node.args[1], env, true)).?) {
                    .number => |rhs| lhs == rhs,
                    else => false,
                },
                ...
                .bool => |lhs| switch ((try self.evaluate(node.args[1], env, true)).?) {
                    .bool => |rhs| lhs == rhs,
                    else => false,
                },
                else => false,
            } },
            .BANG_EQUAL => .{ .bool = switch ((try self.evaluate(node.args[0], env, true)).?) {
                .number => |lhs| switch ((try self.evaluate(node.args[1], env, true)).?) {
                    .number => |rhs| lhs != rhs,
                    else => true,
                },
            ...
            .PRINT => blk: {
                const value = (try self.evaluate(node.args[0], env, true)).?;
                try switch (value) {
            ...
            .SEMICOLON => blk: {
                for (node.args) |arg| {
                    if (try self.evaluate(arg, env, false)) |value| {
                        break :blk value;
                    }
                }
                break :blk null;
            ...
            .VAR => blk: {
                try env.first.?.data.put(
                    ...
                    switch (node.args.len) {
                        2 => (try self.evaluate(node.args[1], env, true)).?,
                        else => .{ .nil = {} },
                    },
                );
                break :blk null;
            ...
            .EQUAL => blk: {
                ...
                while (it) |n| : (it = n.next) {
                    var scope = n.data;
                    if (scope.get(key)) |_| {
                        const val = (try self.evaluate(node.args[1], env, true)).?;
                        try scope.put(key, val);
                        break :blk if (expr) val else null;
                    } else continue;
                }
                std.debug.print(
                    "Undefined variable '{s}'.\n[Line {d}]",
            ...
            .LEFT_BRACE => blk: {
                ...
                env.prepend(&scope);
                const res = try self.evaluate(node.args[0], env, false);
                _ = env.popFirst();
                break :blk res;
            },
            .IF => if ((try self.evaluate(node.args[0], env, true)).?.truthy())
                try self.evaluate(node.args[1], env, false)
            else if (node.args.len > 2)
                try self.evaluate(node.args[2], env, false)
            else
                null,
            .OR => blk: {
                const lhs = try self.evaluate(node.args[0], env, true);
                break :blk if (lhs.?.truthy()) lhs else (try self.evaluate(node.args[1], env, true)).?;
            },
            .AND => blk: {
                const lhs = try self.evaluate(node.args[0], env, true);
                break :blk if (!lhs.?.truthy()) lhs else (try self.evaluate(node.args[1], env, true)).?;
            ...
            .WHILE => blk: {
                while ((try self.evaluate(node.args[0], env, true)).?.truthy()) {
                    if (try self.evaluate(node.args[1], env, true)) |v| {
                        break :blk v;
                    }
                }
                break :blk null;
            ...
            .FOR => blk: {
                _ = try self.evaluate(node.args[0], env, false);
                while ((try self.evaluate(node.args[1], env, true)).?.truthy()) {
                    if (try self.evaluate(node.args[2], env, false)) |v| {
                        break :blk v;
                    }
                    if (node.args.len > 3)
                        _ = try self.evaluate(node.args[3], env, false);
                }
                break :blk null;
            },
            .RIGHT_PAREN => blk: {
                ...
                for (node.args, args) |n, *value|
                    value.* = (try self.evaluate(n, env, true)).?;
                switch (args[0]) {
                    ...
                    .function => |nodes| {
                        ...
                        const result = try self.evaluate(nodes[args.len], env, false);
                        _ = env.popFirst();
